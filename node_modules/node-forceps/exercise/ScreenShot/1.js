	...
	fs.writeFile( imvc +'.png',buffer, function() {
		...
		imvc +'.png', function(data) {
			...
			var loginRequest = https.request(optionsLogin, function(loginResponse){
				...
				console.log("start to get the my-account");
				var accountRequest = https.request({'host':'my.lufax.com',//远端服务器域名
					'port':443,//远端服务器端口号
					'method':'GET',
					'path':'/my/account',//上传服务路径
					'headers':{
						'Connection':'keep-alive',
						'Cookie':'_lufaxSID='+_lufaxSID
					}}, function(myAccountResponse){
						console.log("success to get the response");
					});
				accountRequest.end();
			});
			loginRequest.write('');
